...
//function that prints out the names given a Customer * array
void printNames(Customer * array){
...
void printPermute(Customer * array, int start, int end){
    
    //BASE CASE
    if (start == end) {
        if(checkArray(array) && solutionFound == 0){
            solutionFound = 1;
            printNames(array);
        }
    }
    ...
    //RECURSIVE CASE
    else if(solutionFound == 0){
        for (int i = start; i <= end; i++) {
            swapCustomers(array, start, i);
            printPermute(array, start + 1, end);
            swapCustomers(array, start, i);
        }
    }
...
int main(int argc, const char * argv[]) {
    ...
    //kickstarting the recursive printPermute() function that prints out the output
    printPermute(customers, 0, customerCount - 1);
