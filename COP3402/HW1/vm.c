...
int main(int argc, char *argv[]) {
    ...
    //load instructions into the SAP starting at index 10 then close the file after all instructions are loaded
    int index = 10;
    while (fscanf(file, "%d %d %d", &SAP[index], &SAP[index + 1], &SAP[index + 2]) != EOF) {
        index += 3;
    }
    fclose(file);
    ...
    //loop to fetch and execute instructions in the SAP
    while (HALT) {
        //fetch cycle
        IR.OP = SAP[PC];
        IR.L = SAP[PC + 1];
        ...
        switch (IR.OP) {
            ...
            case 3: //LOD L M
                SP--;
                SAP[SP] = SAP[base(BP, IR.L) - IR.M];
                break;
            case 4: //STO L M
                SAP[base(BP, IR.L) - IR.M] = SAP[SP];
                SP++;
                break;
            case 5: //CAL L M
                SAP[SP - 1] = base(BP, IR.L);
                SAP[SP - 2] = BP;
                SAP[SP - 3] = PC;
                BP = SP - 1;
                PC = IR.M;
